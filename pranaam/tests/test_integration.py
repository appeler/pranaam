...
class TestRealisticScenarios:
    ...
    def test_mixed_religion_predictions(self, mock_model, mock_load_model):
        ...
        def mock_predict(names, verbose=0):
            ...
            for name in names:
                ...
                if any(muslim_indicator in name_lower for muslim_indicator in ['mohammed', 'khan', 'ali', 'shah']):
                    results.append([0.15, 0.85])  # High Muslim probability
                elif any(hindu_indicator in name_lower for hindu_indicator in ['sharma', 'bachchan', 'gupta']):
        ...
        assert result.iloc[2]['pred_label'] in ["muslim", "not-muslim"]  # John Smith (neutral)
    ...
    def test_probability_ranges(self, mock_model, mock_load_model):
        ...
        for i, expected_prob in enumerate(expected_muslim_probs):
            ...
            assert abs(actual_prob - expected_prob) <= 1  # Allow for rounding differences
